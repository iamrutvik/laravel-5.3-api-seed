					...
					case "ajax":
						...
						if(!skipAjaxValidation)
							...
							isAjaxValidator = true;
                                                ...
                                                //console.log('mitesh'+errorMsg+skipAjaxValidation);
						break;
					case "minSize":
...
//			if (options.custom_error_messages[id] && typeof(options.custom_error_messages[id]) != "undefined" &&
		...
		*/
		_customRegex: function(field, rules, i, options) {
                        if(field.val() != '')
                        {
                            ...
                            if(!rule) {
                                    myAlert("jqv:custom rule not found "+customRule);
                                    return;
                            }

                            var ex=rule.regex;
                            if(!ex) {
                                    myAlert("jqv:custom regex not found "+customRule);
                                    return;
                            }
		...
		*/
		_min: function(field, rules, i, options) {
			...
			if (len < min) {
				...
				if (rule.alertText2) return rule.alertText + rule.alertText2;
				 ...
				 $.ajax({
					 ...
					 success: function(json) {
						 ...
						 errorField.trigger("jqv.field.result", [errorField, options.isError, msg]);
                                             if(options.isError==false)    
                                             return options.allrules.ajax.alertText;
					 }
				 });
			 }
		 },
		...
		*/
		_ajaxError: function(data, transport) {
			if(data.status == 0 && transport == null)
				myAlert("The page is not served from a server! ajax call failed");
		 ...
		 _closePrompt: function(field) {
			 ...
			 if (prompt)
				 prompt.fadeTo("fast", 0, function() {
					 ...
					 prompt.remove();
                                         field.removeClass('errorMsgTxtBx');
				 });
		 },
